...
public class PortalTravelAgentAether extends PortalTravelAgent {
    ...
    public Location findPortal(Location location) {
        ...
        for (int j1 = l - this.searchRadius; j1 <= l + this.searchRadius; ++j1) {
            ...
            for (int k1 = i1 - this.searchRadius; k1 <= i1 + this.searchRadius; ++k1) {
                ...
                for (int l1 = 127; l1 >= 0; --l1)
                    if (world.getTypeId(j1, l1, k1) == BlockManager.Portal.id) {
                        while (world.getTypeId(j1, l1 - 1, k1) == BlockManager.Portal.id)
                            --l1;
        ...
        if (d0 >= 0.0D) {
            ...
            d1 = (double) k + 0.5D;
            if (world.getTypeId(i - 1, j, k) == BlockManager.Portal.id)
                d5 -= 0.5D;

            if (world.getTypeId(i + 1, j, k) == BlockManager.Portal.id)
                d5 += 0.5D;

            if (world.getTypeId(i, j, k - 1) == BlockManager.Portal.id)
                d1 -= 0.5D;
            ...
            for (l2 = -1; l2 <= 1; ++l2)
                for (k2 = 1; k2 < 3; ++k2)
                    for (j3 = -1; j3 < 3; ++j3) {
                        ...
                        k3 = j2 + (k2 - 1) * l5 - l2 * k5;
                        flag = j3 < 0;
                        world.setTypeId(i3, l3, k3, flag ? Block.GLOWSTONE.id : 0);
                    }
        }
        ...
        for (l2 = 0; l2 < 4; ++l2) {
            ...
            for (k2 = 0; k2 < 4; ++k2)
                for (j3 = -1; j3 < 4; ++j3) {
                    ...
                    world.setTypeId(i3, l3, k3, flag ? Block.GLOWSTONE.id : BlockManager.Portal.id);
                }
