...
public abstract class EntityProjectileBase extends Entity implements ISpawnable {
	...
	public EntityProjectileBase(World world, EntityLiving entityliving) {
		...
		setPositionRotation(entityliving.locX, entityliving.locY + (double) entityliving.t(), entityliving.locZ,
	...
	public void setArrowHeading(double d, double d1, double d2, float f, float f1) {
		...
		packet.dataFloat = new float[] { (float) d, (float) d1, (float) d2, f, f1 };
		packet.packetType = 9;
		packet.modId = ModLoaderMp.GetModInstance(mod_AetherMp.class).getId();
		ModLoader.getMinecraftServerInstance().serverConfigurationManager.sendPacketNearby(locX, locY, locZ, 64,
				((WorldServer) world).dimension, packet);
	}
	...
	public void m_() {
		...
		if (movingobjectposition != null && onHit()) {
			...
			if (ent != null && onHitTarget(ent)) {
				boolean stick = true;
				if (ent instanceof EntityLiving) {
					Server server = this.world.getServer();
					org.bukkit.entity.Entity damagee = ent.getBukkitEntity();
					Projectile projectile = (Projectile) this.getBukkitEntity();
					EntityDamageByEntityEvent event = new EntityDamageByEntityEvent(projectile, damagee,
							EntityDamageEvent.DamageCause.PROJECTILE, dmg);
					server.getPluginManager().callEvent(event);
					this.shooter = ((projectile.getShooter() == null) ? null
							: ((CraftLivingEntity) projectile.getShooter()).getHandle());
		...
		for (pitch = (float) ((Math.atan2(motY, f2) * 180D) / 3.1415927410125732D); pitch
	...
	public void b(EntityHuman entityhuman) {
		...
		if (this.inGround && this.shotByPlayer && this.arrowShake <= 0 && entityhuman.inventory.canHold(item) > 0) {
			...
			final PlayerPickupItemEvent event = new PlayerPickupItemEvent((Player) entityhuman.getBukkitEntity(),
					new CraftItem(this.world.getServer(), entityitem), 0);
		...
		if (inGround && shotByPlayer && arrowShake <= 0 && entityhuman.inventory.pickup(item.cloneItemStack())) {
			if (entityhuman instanceof EntityPlayer && !dead) {
				EntityTracker entitytracker = PackageAccess.EntityPlayer.getMCServer((EntityPlayer) entityhuman)
						.getTracker(entityhuman.dimension);
